...
print(len(tokenizer.word_index)) # this print the unique number of words in dataset
X_train_seq = tokenizer.texts_to_sequences(X_train)  # Convert to sequences
X_test_seq = tokenizer.texts_to_sequences(X_test)

X_train_padded = pad_sequences(X_train_seq,31,  padding='post')
X_test_padded = pad_sequences(X_test_seq,31, padding='post')
...
def predict_sentiment(text):
    ...
    padd = pad_sequences(sequence,31,padding='post')
